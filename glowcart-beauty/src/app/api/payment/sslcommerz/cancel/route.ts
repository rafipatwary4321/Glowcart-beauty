...
export const GET = withDb(async (request: Request) => handleCancel(request));
export const POST = withDb(async (request: Request) => handleCancel(request));
...
async function handleCancel(request: Request) {
  ...
  if (orderId) {
    ...
    return NextResponse.redirect(
      `${appUrl}${routes.checkout}?payment=cancelled&orderId=${orderId}`
    );
  }

  return NextResponse.redirect(`${appUrl}${routes.checkout}?payment=cancelled`);
}
